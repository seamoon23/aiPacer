...
다음 초기화는 선택한 요일의 다음 도래 시점 0시로 봅니다. 현재 시각부터 초기화까지 남은 주 사용시간에 주간 용량을 나누고, 오늘 남은 사용시간 비율만큼 오늘 권장 용량을 계산합니다.
...
브라우저에서 터미널에 표시된 주소를 열고 `/ai-pacer/`로 이동합니다. `npm run dev`는 Vite 의존성 캐시를 강제로 다시 만들어 정적인 화면만 보이는 하이드레이션 실패를 예방합니다.
